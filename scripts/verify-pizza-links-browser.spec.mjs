...
for (const language of ["en", "he"]) {
  test(`${language}: pizza resources work and calculator stays out of global navigation`, async ({
    ...
    const calculator = body.locator(
      `a[href="${productionBase}${calculatorPath}"]`,
    );
    ...
    const navigation = page.waitForResponse(
      (result) =>
        result.url() === `${productionBase}${calculatorPath}` &&
    ...
    await expect(page.locator('[data-field="desiredDough"]')).toBeVisible();
