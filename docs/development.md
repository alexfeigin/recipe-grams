...
and, rarely, Apple's Install window. **Ready** means the baseline is present on
a supported host. It does not mean that `npm run verify` has passed, that
personal skills exist, or that publication is authorized.
