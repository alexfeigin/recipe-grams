...
tracked image source in `public/images/`, let Astro copy it to `/images/`, and
generate root-level compatibility copies at build time for previously published
image URLs. This supersedes ADR 0036 and the image-hosting progression in
...
shared assets, and social previews. Build-generated copies also retain every
old `/recipe-grams/...` image URL, including unreferenced images. Keep those
images unless positive evidence supports removal; repository searches cannot
prove a published URL has no readers. The retained title icons and pattern
reference are explained in historical ADR 0028.
